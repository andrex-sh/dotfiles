...
install_pacman() {
    pkgs="sway swaybg swaylock swayidle i3status foot fuzzel lxqt-policykit kanshi mako grim slurp wl-clipboard playerctl brightnessctl libpulse ttf-jetbrains-mono-nerd yazi tmux jq"
    missing=""
    for p in $pkgs; do
...
install_apt() {
    pkgs="sway swaybg swaylock swayidle i3status foot fuzzel lxqt-policykit kanshi mako-notifier grim slurp wl-clipboard playerctl brightnessctl pulseaudio-utils wget tar fontconfig ca-certificates tmux jq"
    missing=""
    for p in $pkgs; do
